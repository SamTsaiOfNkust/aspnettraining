...
namespace KUAS.Dapper
{
        ...
        where TEntity : class
    {
        ...
        /// Sql provider
        /// </summary>
        public EDbConnector SqlConnector { get; set; }
        ...
        /// Constructor
        /// </summary>
        public SqlGenerator(EDbConnector sqlConnector)
        {
            InitProperties();
        ...
        private void InitProperties()
        {
            ...
            if (dateChangedProperty != null && (dateChangedProperty.PropertyType == typeof(DateTime) || dateChangedProperty.PropertyType == typeof(DateTime?)))
                dateChangedProperty = props.FirstOrDefault(p => p.GetCustomAttributes<UpdatedAtAttribute>().Any());
        }
        ...
        private void InitSqlConnector(EDbConnector sqlConnector)
        {
        ...
        /// 批次更新
        /// </summary>
        /// <param name="entities">多個資料模型</param>
        /// <returns></returns>
        public virtual SqlQuery GetInsertMany(IEnumerable< TEntity> entities)
        {
            var properties = (this.IsIdentity ? this.SqlProperties.Where(p => !p.Name.Equals(this.IdentitySqlProperty.Name, StringComparison.OrdinalIgnoreCase)) : this.SqlProperties).ToList();

            if (HasDateChanged)
            {
                DateChangedProperty.SetValue(entities, DateTime.UtcNow);
            }
            ...
            return new SqlQuery(sqlBuilder.ToString(), entities);
        }
        ...
        public virtual SqlQuery GetUpdateMany(IEnumerable< TEntity> entities)
        {
            var properties = SqlProperties.Where(p => !KeySqlProperties.Any(k => k.Name.Equals(p.Name, StringComparison.OrdinalIgnoreCase)));
            ...
            return new SqlQuery(sqlBuilder.ToString().TrimEnd(), entities);
        }
        #endregion Update
        ...
        public virtual SqlQuery GetDeleteMany(IEnumerable< TEntity> entities)
        {
            var sqlBuilder = new StringBuilder();
        ...
        /// <summary>
        /// Retuen Result With Where Predicate
        /// </summary>
        ...
        /// <summary>
        ///  Get Result with Join Tables 
        /// </summary>
        /// <param name="originalBuilder"></param>
        /// <param name="includes"></param>
        /// <returns></returns>
        ...
        /// <summary>
        /// Return Result With Where Predicate
        /// </summary>
        /// <param name="predicate"></param>
        ...
        private SqlQuery GetSelect(Expression<Func<TEntity, bool>> predicate, bool firstOnly, params Expression<Func<TEntity, object>>[] includes)
        {
            ...
            if (firstOnly && (SqlConnector == EDbConnector.Sybase))
                builder.Append("Set Rowcount 1");
        ...
        private StringBuilder InitBuilderSelect(bool firstOnly)
        {
            ...
            if (firstOnly && SqlConnector == EDbConnector.Mssql)
                select += "TOP 1 ";
